...
class RowCounterTest extends PHPUnit_Framework_TestCase
{
    ...
    protected function getMockStatement($sql, $returnValue)
    {
        ...
        $statement = $this->getMock('Zend\Db\Adapter\Driver\Oci8\Statement', array('prepare', 'execute'), array(), '', false);

        // mock the result
        $result = $this->getMock('stdClass', array('current'));
    ...
    {
        $oci8Statement = $this->getMock('stdClass', array('current'), array(), '', false); // stdClass can be used here
        ...
        $driver = $this->getMock('Zend\Db\Adapter\Driver\Oci8\Oci8', array('getConnection'), array(), '', false);
