...
class StreamWriterTest extends \PHPUnit_Framework_TestCase
{
    public function testConstructorThrowsWhenResourceIsNotStream()
    {
        ...
        try {
            ...
            $this->assertType('Zend\Log\Writer\Exception\InvalidArgumentException', $e);
    ...
    {
        $stream = fopen('php://memory', 'w+');
        $this->setExpectedException('Zend\Log\Writer\Exception\InvalidArgumentException', 'existing stream');
        new StreamWriter($stream, 'w+');
    }

    public function testConstructorThrowsWhenStreamCannotBeOpened()
    {
        $this->setExpectedException('Zend\Log\Writer\Exception\RuntimeException', 'cannot be opened');
        new StreamWriter('');
    }
    ...
    public function testWriteThrowsWhenStreamWriteFails()
    {
        ...
        $this->setExpectedException('Zend\Log\Writer\Exception\RuntimeException', 'Unable to write');
