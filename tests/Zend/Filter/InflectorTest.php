...
class InflectorTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function testFilterTransformsStringAccordingToRules()
    {
        $this->inflector->setTarget(':controller/:action.:suffix')
             ->addRules(array(
                 ':controller' => array('wordCamelCaseToDash'),
                 ':action'     => array('wordCamelCaseToDash'),
                 'suffix'      => 'phtml'
             ));
    ...
    public function testTargetExceptionNotThrownOnIdentifierNotFollowedByCharacter()
    {
        $inflector = new InflectorFilter(
            ...
            array(
                 ':controller' => array('wordCamelCaseToDash', 'stringToLower'),
                 ':action'     => array('wordCamelCaseToDash'),
                 'suffix'      => 'phtml'
                ),
            true,
    ...
    public function getOptions()
    {
        $options = array(
            ...
            'rules' => array(
                ':controller' => array(
                    'rule1' => 'wordCamelCaseToUnderscore',
                    'rule2' => 'stringToLower',
                ),
                ':action' => array(
                    'rule1' => 'wordCamelCaseToDash',
                    'rule2' => 'stringToUpper',
                ),
                'suffix' => 'php'
            ),
