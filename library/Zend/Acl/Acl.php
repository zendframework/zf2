...
class Acl
{
     ...
     * indicate the Roles from which the newly added Role will directly
     * inherit.
     *
     * In order to resolve potential ambiguities with conflicting rules 
     * inherited from different parents, the most recently added parent takes
     * precedence over parents that were previously added. In other words, the 
     * first parent added will have the least priority, and the last parent 
     * added will have the highest priority.
     *
    ...
    {
        return $this->_getRoleRegistry()->inherits(
            $role, $inherit, $onlyParents
        );
    }
    ...
    public function removeRole($role)
    {
        ...
        foreach ($this->_rules['allResources']['byRoleId'] 
            as $roleIdCurrent => $rules) {
        ...
        foreach ($this->_rules['byResourceId'] 
            as $resourceIdCurrent => $visitor) {
     ...
     * for, the existing Resource from which the newly added Resource will
     * inherit.
     *
    ...
    public function addResource($resource, $parent = null)
    {
        ...
        if (!$resource instanceof Resource) {
            throw new Exception\InvalidArgumentException(
                'addResource() expects $resource to be of 
                type Zend_Acl_Resource_Interface'
            );
        }
        ...
        if (null !== $parent) {
            try {
                ...
                throw new Exception\InvalidArgumentException(
                    "Parent Resource id '$resourceParentId' does not exist", 
                    0, $e
                );
            }
    ...
    public function inheritsResource($resource, $inherit, $onlyParent = false)
    {
        ...
        if (null !== $this->_resources[$resourceId]['parent']) {
            $parentId = $this->_resources[$resourceId]['parent']
                ->getResourceId();
    ...
    public function removeResource($resource)
    {
        ...
        if (null !== 
            ($resourceParent = $this->_resources[$resourceId]['parent'])) {
            unset(
                $this->_resources[$resourceParent->getResourceId()]
                ['children'][$resourceId]
            );
        }
        foreach ($this->_resources[$resourceId]['children'] 
            as $childId => $child) {
    ...
    public function allow(
        $roles = null, $resources = null, 
        $privileges = null, Assertion $assert = null)
    {
        return $this->setRule(
            self::OP_ADD, self::TYPE_ALLOW, $roles, 
    ...
    {
        return $this->setRule(
            self::OP_ADD, self::TYPE_DENY, $roles, 
            $resources, $privileges, $assert
        );
    }
    ...
    public function removeAllow(
        $roles = null, $resources = null, $privileges = null)
    {
        return $this->setRule(
            self::OP_REMOVE, self::TYPE_ALLOW, $roles, $resources, $privileges
        );
    }
    ...
    {
        return $this->setRule(
            self::OP_REMOVE, self::TYPE_DENY, $roles, $resources, $privileges
        );
    }
     ...
     *
     * The $operation parameter may be either OP_ADD or OP_REMOVE, depending on
     * whether the user wants to add or remove a rule, respectively:
     *
     * OP_ADD specifics:
     *
     ...
     *      Existing rules to which the remove operation does not apply would 
     *      remain in the ACL.
     *
     * The $type parameter may be either TYPE_ALLOW or TYPE_DENY, depending on 
     * whether the rule is intended to allow or deny permission, respectively.
     *
     * The $roles and $resources parameters may be references to, or the string
     * identifiers for, existing Resources/Roles, or they may be passed as 
     * arrays of these - mixing string identifiers and objects is ok - to 
     * indicate the Resources and Roles to which the rule applies. If either
     * $roles or $resources is null, then the rule applies to all Roles or all
     * Resources, respectively. Both may be null in order to work with the 
     * default rule of the ACL.
     *
     * The $privileges parameter may be used to further specify that the rule 
     * applies only to certain privileges upon the Resource(s) in question. 
     * This may be specified to be a single privilege with a string, and 
     * multiple privileges may be specified as an array of strings.
     *
     * If $assert is provided, then its assert() method must return true in 
     * order for the rule to apply. If $assert is provided with $roles, 
     * $resources, and $privileges all equal to null, then a rule having a type 
     * of:
     *
     ...
     * provide expected behavior when an assertion upon the default ACL rule 
     * fails.
     *
     * @uses   Zend\Acl\Role\Registry::get()
    ...
    public function setRule(
        $operation, $type, $roles = null, $resources = null, 
        $privileges = null, Assertion $assert = null) 
    {
        ...
        // ensure that all specified Roles exist; normalize input to array of 
        // Role objects or null
        if (!is_array($roles)) {
            $roles = array($roles);
        ...
        // ensure that all specified Resources exist; normalize input to array 
        // of Resource objects or null
                        ...
                        if (0 === count($privileges)) {
                            ...
                            foreach ($privileges as $privilege) {
                                $rules['byPrivilegeId'][$privilege]['type']   = 
                                    $type;
                                $rules['byPrivilegeId'][$privilege]['assert'] = 
                                    $assert;
                            }
                        }
                    }
     ...
     * identifiers for, an existing Resource and Role combination.
     *
     * If either $role or $resource is null, then the query applies to all 
     * Roles or all Resources, respectively. Both may be null to query whether
     * the ACL has a "blacklist" rule (allow everything to all). By default, 
     * Zend_Acl creates a "whitelist" rule (deny everything to all), and this 
     * method would return false unless this default has been overridden 
     * (i.e., by executing $acl->allow()).
     *
     * If a $privilege is not provided, then this method returns false if and
     * only if the Role is denied access to at least one privilege upon the 
     * Resource. In other words, this method returns true if and only if the 
     * Role is allowed all privileges on the Resource.
     *
     * This method checks Role inheritance using a depth-first traversal of 
     * the Role registry. The highest priority parent (i.e., the parent most 
     * recently added) is checked first, and its respective parents are checked 
     * similarly before the lower-priority parents of the Role are checked.
     *
     * @uses   Zend\Acl\Acl::get()
     * @uses   Zend\Acl\Role\Registry::get()
     * @param  Zend\Acl\Role|string     $role
    ...
    public function isAllowed($role = null, $resource = null, $privilege = null)
    {
        ...
        if (null === $privilege) {
            ...
            do {
                ...
                // pseudo-parent
                if (null !== $role && null !== 
                    ($result = $this->_roleDFSAllPrivileges(
            ...
            do {
                ...
                // pseudo-parent
                if (null !== $role && null !== 
                    ($result = $this->_roleDFSOnePrivilege(
                        $role, $resource, $privilege
                    ))) {
                    return $result;
                }
    ...
    /**
     * Performs a depth-first search of the Role DAG, starting at $role, in 
     * order to find a rule allowing/denying $role access to all privileges 
     * upon $resource
     *
     * This method returns true if a rule is found and allows access. If a rule 
     * exists and denies access, then this method returns false. If no 
    ...
    protected function _roleDFSAllPrivileges(
        Role $role, Resource $resource = null)
    {
        $dfs = array(
            'visited' => array(),
            'stack'   => array()
        ...
        while (null !== ($role = array_pop($dfs['stack']))) {
            if (!isset($dfs['visited'][$role->getRoleId()])) {
                if (null !== ($result = $this->_roleDFSVisitAllPrivileges(
                    $role, $resource, $dfs
                ))) {
                    return $result;
                }
     ...
     * applicable rule is found, then this method returns null.
     *
     * This method is used by the internal depth-first search algorithm and may
     * modify the DFS data structure.
     *
     * @param  Zend\Acl\Role     $role
     * @param  Zend\Acl\Resource $resource
     * @param  array             $dfs
    ...
    protected function _roleDFSVisitAllPrivileges(
        Role $role, Resource $resource = null, &$dfs = null)
    {
        if (null === $dfs) {
        ...
        foreach ($this->_getRoleRegistry()->getParents($role) 
            as $roleParentId => $roleParent) {
    ...
    /**
     * Performs a depth-first search of the Role DAG, starting at $role, in 
     * order to find a rule allowing/denying $role access to a $privilege upon 
     * $resource
     *
    ...
    protected function _roleDFSOnePrivilege(
        Role $role, Resource $resource = null, $privilege = null)
    {
        if (null === $privilege) {
            throw new Exception\RuntimeException(
        ...
        if (null !== ($result = $this->_roleDFSVisitOnePrivilege(
            $role, $resource, $privilege, $dfs
        ))) {
            return $result;
        }
    ...
    protected function _roleDFSVisitOnePrivilege(
        Role $role, Resource $resource = null, $privilege = null, &$dfs = null) 
    {
        if (null === $privilege) {
            /**
    ...
    /**
     * Returns the rule type associated with the specified Resource, Role, and 
     * privilege combination.
     *
     * If a rule does not exist or its attached assertion fails, which means 
     * that the rule is not applicable, then this method returns null. 
     * Otherwise, the rule type applies and is returned as either TYPE_ALLOW or 
     * TYPE_DENY.
     *
     * If $resource or $role is null, then this means that the rule must apply
     * to all Resources or Roles, respectively.
     *
     * If $privilege is null, then the rule must apply to all privileges.
     *
     * If all three parameters are null, then the default ACL rule type is 
     * returned, based on whether its assertion method passes.
     *
     * @param  Zend\Acl\Resource $resource
     * @param  Zend\Acl\Role     $role
     * @param  string            $privilege
    ...
    protected function _getRuleType(
        Resource $resource = null, Role $role = null, $privilege = null)
    {
        ...
        if ($rule['assert']) {
            ...
            $assertionValue = $assertion->assert(
                $this,
                ($this->_isAllowedRole instanceof Role) 
                ? $this->_isAllowedRole : $role,
                ($this->_isAllowedResource instanceof Resource) 
                ? $this->_isAllowedResource : $resource,
    ...
    /**
     * Returns the rules associated with a Resource and a Role, or null if no 
     * such rules exist
     *
     * If either $resource or $role is null, this means that the rules returned 
     * are for all Resources or all Roles, respectively. Both can be null to 
     * return the default rule set for all Resources and all Roles.
     *
     * If the $create parameter is true, then a rule set is first created and 
    ...
    protected function &_getRules(
        Resource $resource = null, Role $role = null, $create = false)
    {
