...
abstract class AbstractTable
{
     ...
     * setDefinition()
     *
     * @param \Zend\Db\Table\Definition $definition
     * @return \Zend\Db\Table\AbstractTable
     */
     ...
     * getDefinition()
     *
     * @return \Zend\Db\Table\Definition|null
     */
    ...
    public function getReference($tableClassname, $ruleKey = null)
    {
        ...
        foreach ($refMap as $reference) {
            if ($reference[self::REF_TABLE_CLASS] == '\\' . ltrim($tableClassname, '\\')) {
                return $reference;
            }
        }
     ...
     *
     * @return \Zend\Db\Adapter\AbstractAdapter or null
     */
    ...
    protected function _setupDatabaseAdapter()
    {
        if (! $this->_db) {
            ...
            if (!$this->_db instanceof Adapter\AbstractAdapter) {
    ...
    protected function _setupPrimaryKey()
    {
        if (!$this->_primary) {
            ...
            if (empty($this->_primary)) {
                throw new Exception('A table must have a primary key, but none was found');
            }
         ...
         */
        if ($this->_sequence === true && $this->_db instanceof \Zend\Db\Adapter\PDO\PGSQL) {
    ...
    public function info($key = null)
    {
        ...
        if (!array_key_exists($key, $info)) {
            throw new Exception('There is no table information for the key "' . $key . '"');
        }
