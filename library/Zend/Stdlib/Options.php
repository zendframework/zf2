...
abstract class Options implements ParameterObjectInterface
{
    ...
    protected function assembleSetterNameFromKey($key)
    {
        ...
        if (!method_exists($this, $setter)) {
            throw new Exception\BadMethodCallException(
                "The option '{$key}' does not have a matching '{$setter}' setter method which must be defined"
            );
        }
        return $setter;
    }
    ...
    protected function assembleGetterNameFromKey($key)
    {
        ...
        if (!method_exists($this, $getter)) {
            throw new Exception\BadMethodCallException(
                "The option '{$key}' does not have a matching '{$getter}' getter method which must be defined"
            );
        }
        return $getter;
    }
