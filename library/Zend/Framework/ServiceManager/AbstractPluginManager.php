...
abstract class AbstractPluginManager extends ServiceManager implements ServiceLocatorAwareInterface
{
    ...
    {
        $name = strtolower($service->getName());

        if (!isset($this->invokableClasses[$name])) {
            return;
        }

        if (!parent::getConfig($name)) {
            var_dump($name, $this->invokableClasses[$name]);
            parent::addInvokableClass($name, $this->invokableClasses[$name]);
        }
